...
/*	ull r[2],a[2],b[2],p[2];
	...
	AsmMDMulX f(p[1],p[0],p[0]);
	void (*f_mul)(ull *,ull,ull)=(void (*)(ull *,ull,ull))f.getCode();
	AsmMDMul f2(p[1],p[0],p[0]);
	void (*f_mul_2)(ull &,ull &,ull,ull)=(void (*)(ull &,ull &,ull,ull))f2.getCode();
	r[1]=a[1];
	r[0]=a[0];
//	double start=get_usage_sec();
	f_mul(r,b[1],b[0]);
//	printf("time: %f\n",get_usage_sec()-start);
	cout<<" a[1]: "<<a[1]<<" a[0]: "<<a[0]<<endl;
	cout<<"+b[1]: "<<b[1]<<" b[0]: "<<b[0]<<endl;
	cout<<"=r[1]: "<<r[1]<<" r[0]: "<<r[0]<<endl;

	r[1]=a[1];
	r[0]=a[0];
	f_mul_2(r[1],r[0],b[1],b[0]);
	cout<<"=r[1]: "<<r[1]<<" r[0]: "<<r[0]<<" true"<<endl;*/
